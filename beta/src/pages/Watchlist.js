...
export function Watchlist(state){
 ...
 ${items.length?`<div class="stack">${items.map(x=>mediaCard(state,x)).join("")}</div>`:`<div class="empty-state"><h2>Your watchlist is empty.</h2><p>Media Minder will keep the good stuff easy to find.</p><button class="btn" data-route="recommendations">See recommendations</button></div>`}`;
}
