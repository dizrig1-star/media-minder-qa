...
export function Settings(state){
 ...
 <p><strong>Favorite celebrations</strong></p><div class="cluster">${state.profile.favoriteCelebrations.map(x=>`<span class="platform">${x}</span>`).join("")}</div>
 <p><strong>Preferred platforms</strong></p><div class="cluster">${state.profile.platforms.map(x=>`<span class="platform">${state.platforms.find(p=>p.id===x)?.name||x}</span>`).join("")}</div>
