...
export function Premieres(state){
  ...
  <div class="grid-2"><section class="stack editorial-stack">${items.length?items.map(({show})=>EditorialCard(show,platformName(state,show.platform),"premiere",state,undefined,show.mmSelect?['select']:[])).join(""):"<div class='empty-state'><h2>No qualifying premieres yet.</h2><p>Media Minder will surface relevant new series and season openers as they approach.</p></div>"}</section>
